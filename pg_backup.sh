...
if [ -z $CONFIG_FILE_PATH ] ; then
        ...
        CONFIG_FILE_PATH="/scripts/pg_backup/pg_backup.config"
fi
...
function perform_backups()
{
        ...
        if  [ $ENABLE_ALL_DB_BACKUPS = "yes" ]; then

              FULL_BACKUP_QUERY="select datname from pg_database where not datistemplate and datallowconn $EXCLUDE_SCHEMA_ONLY_CLAUSE order by datname;"
              DATABASE_LIST='psql -h "$HOSTNAME" -p 5000 -U "$USERNAME" -At -c "$FULL_BACKUP_QUERY" postgres'
        else
                DATABASE_LIST=$DB_BACKUP_LIST
        fi
...
DAY_OF_MONTH=`date +%d`
EXPIRED_MONTH=`expr $((($MONTH_TO_KEEP * 30) + 1))`

if [ $DAY_OF_MONTH -eq 1 ];
then
        # Delete all expired monthly directories
        find $BACKUP_DIR -maxdepth 1 -mtime +$EXPIRED_MONTH -name "*-monthly" -exec rm -rf '{}' ';'
